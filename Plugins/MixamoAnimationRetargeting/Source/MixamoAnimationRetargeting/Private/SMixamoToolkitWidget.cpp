...
void SRiggedSkeletonPicker::Construct(const FArguments& InArgs)
{
	...
	UObject* DefaultSkeletonObj = USkeleton::StaticClass()->GetDefaultObject();
	FAssetRegistryTagsContextData TagsContext(DefaultSkeletonObj, EAssetRegistryTagsCaller::Uncategorized);
	DefaultSkeletonObj->GetAssetRegistryTags(TagsContext);
...
TSharedRef<SWidget> SRootMotionExtractionWidget::CreateAnimationSequencePicker(const USkeleton * ReferenceSkeleton, bool InPlaceAnimation)
{
    ...
    AssetPickerConfig.bShowTypeInColumnView = false;
    // Hide all asset registry columns by default (we only really want the name and path)
	UObject* DefaultAnimSequenceObj = UAnimSequence::StaticClass()->GetDefaultObject();
	FAssetRegistryTagsContextData TagsContext(DefaultAnimSequenceObj, EAssetRegistryTagsCaller::Uncategorized);
	DefaultAnimSequenceObj->GetAssetRegistryTags(TagsContext);
	for (const TPair<FName, UObject::FAssetRegistryTag>& TagPair : TagsContext.Tags)
	{
        AssetPickerConfig.HiddenColumnNames.Add(TagPair.Key.ToString());
    }
	...
	for (int32 ExtensionIndex = 0; ExtensionIndex < NumExtensions; ++ExtensionIndex)
	{
		...
		if (DataSource->IsA<UContentBrowserAssetDataSource>())
		{
			for (auto Asset : AssetsToOverwrite)
			{
				...
				InCallback(ContentBrowserAssetData::CreateAssetFileItem(DataSource, VirtualizedPath, NAME_None, AssetData));
			}
