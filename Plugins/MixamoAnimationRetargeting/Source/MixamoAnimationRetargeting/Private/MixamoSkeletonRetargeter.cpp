...
static const FSoftObjectPath kMetaHumanBaseSkeleton_ObjectPath(TEXT("/Game/MetaHumans/Common/Female/Medium/NormalWeight/Body/metahuman_base_skel.metahuman_base_skel"));
static const FSoftObjectPath kMetaHumanDefaultSkeletalMesh_ObjectPath(TEXT("/Game/MetaHumans/Common/Female/Medium/NormalWeight/Body/f_med_nrw_body.f_med_nrw_body"));
...
void ConfigureBonePreferredAnglesLocalToBS(
	...
	UIKRig_FBIKBoneSettings* Settings,
	FName ChildBoneName,
...
void FMixamoSkeletonRetargeter::RetargetToUE4Mannequin(TArray<USkeleton *> Skeletons) const
{
	...
	const bool bIsMetaHumanSkeleton = (
		...
		&& FSoftObjectPath(ReferenceSkeleton->GetPathName()) == kMetaHumanBaseSkeleton_ObjectPath
	);
	...
	if (bIsMetaHumanSkeleton)
	{
		bSetPreviewMesh = (
			...
			|| !(FSoftObjectPath(ReferenceSkeleton->GetPreviewMesh()->GetPathName()) == kMetaHumanDefaultSkeletalMesh_ObjectPath)
		);
	}
#endif
	...
	if (bSetPreviewMesh)
	{
		...
		if (bIsMetaHumanSkeleton)
		{
			auto RepositionSkeletalMesheByObjectPath = [&](const FSoftObjectPath& Query) {
				int32 i = ReferenceSkeletalMeshes.IndexOfByPredicate(
					[&](const FAssetData& A) {
						return A.GetSoftObjectPath() == Query;
					}
				);
				if (i == INDEX_NONE)
				{
			...
			if (!RepositionSkeletalMesheByObjectPath(kMetaHumanDefaultSkeletalMesh_ObjectPath))
			{
				const FString ErrorMessage = FString::Format(TEXT("Default MetaHuman skeletal mesh '{0}' not found. Retargeting aborted."),
					{ kMetaHumanDefaultSkeletalMesh_ObjectPath.ToString() });
...
bool FMixamoSkeletonRetargeter::OnShouldFilterNonUEMannequinSkeletonAsset(const FAssetData& AssetData) const
{
	...
	{
		return !(AssetData.GetSoftObjectPath() == kMetaHumanBaseSkeleton_ObjectPath);
	}
#endif
...
) const
{
	...
	if (ReferenceSkeletonType == ETargetSkeletonType::ST_UE5_MANNEQUIN)
	{
		UEMannequinSkeletalMeshes = UEMannequinSkeletalMeshes.FilterByPredicate([](USkeletalMesh* S) {
			...
			return
				...
				|| FSoftObjectPath(PathName) == kMetaHumanDefaultSkeletalMesh_ObjectPath;
		});
	}
#endif // MAR_IGNORE_UE5_MANNEQUIN
	...
	*/
	...
	//Bug #1080: Redo cache derived data for already loaded animation sequences
	//			 otherwise the retargeting preview is bugged.
	...
	const FText StatusText = FText::Format(LOCTEXT("MixamoSkeletonRetargeter_Compressing", "Compressing '{0}' animations"), FText::AsNumber(AnimSeqsToRecompress.Num()));
...
void FMixamoSkeletonRetargeter::AddRootBone(USkeleton* Skeleton, TArray<USkeletalMesh *> SkeletalMeshes) const
{
	...
	for (int iMesh = 0; iMesh < SkeletalMeshes.Num(); ++ iMesh)
	{
		...
		for (FSkeletalMeshLODModel & LODModel : SkeletonMesh->GetImportedModel()->LODModels)
		{
			...
			if (SkeletonMesh->HasMeshDescription(LODIndex))
			{
...
) const
{
	...
	{
		Controller->CreateRetargetPose(Mesh->GetFName(), ERetargetSourceOrTarget::Target);
	...
	if (USkeletalMesh* PreviewMesh = Controller->GetPreviewMesh(ERetargetSourceOrTarget::Target))
	{
		Controller->SetCurrentRetargetPose(PreviewMesh->GetFName(), ERetargetSourceOrTarget::Target);
	}
	else
	{
		Controller->SetCurrentRetargetPose(Controller->GetAsset()->GetDefaultPoseName(), ERetargetSourceOrTarget::Target);
	}
}
...
UIKRigDefinition* FMixamoSkeletonRetargeter::CreateMixamoIKRig(const USkeleton* Skeleton) const
{
	...
	Controller->AddRetargetChain(TEXT("Root"), TEXT("root"), TEXT("root"), NAME_None);
	Controller->AddRetargetChain(TEXT("Spine"), TEXT("Spine"), TEXT("Spine2"), NAME_None);
	Controller->AddRetargetChain(TEXT("Head"), TEXT("Neck"), TEXT("head"), NAME_None);
	static const FName LeftClavicleChainName(TEXT("LeftClavicle"));
	Controller->AddRetargetChain(LeftClavicleChainName, TEXT("LeftShoulder"), TEXT("LeftShoulder"), NAME_None);
	static const FName LeftArmChainName(TEXT("LeftArm"));
	static const FName LeftHandBoneName(TEXT("LeftHand"));
	Controller->AddRetargetChain(LeftArmChainName, TEXT("LeftArm"), LeftHandBoneName, NAME_None);
	static const FName RightClavicleChainName(TEXT("RightClavicle"));
	Controller->AddRetargetChain(RightClavicleChainName, TEXT("RightShoulder"), TEXT("RightShoulder"), NAME_None);
	static const FName RightArmChainName(TEXT("RightArm"));
	static const FName RightHandBoneName(TEXT("RightHand"));
	Controller->AddRetargetChain(RightArmChainName, TEXT("RightArm"), RightHandBoneName, NAME_None);
	static const FName LeftLegChainName(TEXT("LeftLeg"));
	static const FName LeftToeBaseBoneName(TEXT("LeftToeBase"));
	Controller->AddRetargetChain(LeftLegChainName, TEXT("LeftUpLeg"), LeftToeBaseBoneName, NAME_None);
	static const FName RightLegChainName(TEXT("RightLeg"));
	static const FName RightToeBaseBoneName(TEXT("RightToeBase"));
	Controller->AddRetargetChain(RightLegChainName, TEXT("RightUpLeg"), RightToeBaseBoneName, NAME_None);
	Controller->AddRetargetChain(TEXT("LeftIndex"), TEXT("LeftHandIndex1"), TEXT("LeftHandIndex3"), NAME_None);
	Controller->AddRetargetChain(TEXT("RightIndex"), TEXT("RightHandIndex1"), TEXT("RightHandIndex3"), NAME_None);
	Controller->AddRetargetChain(TEXT("LeftMiddle"), TEXT("LeftHandMiddle1"), TEXT("LeftHandMiddle3"), NAME_None);
	Controller->AddRetargetChain(TEXT("RightMiddle"), TEXT("RightHandMiddle1"), TEXT("RightHandMiddle3"), NAME_None);
	Controller->AddRetargetChain(TEXT("LeftPinky"), TEXT("LeftHandPinky1"), TEXT("LeftHandPinky3"), NAME_None);
	Controller->AddRetargetChain(TEXT("RightPinky"), TEXT("RightHandPinky1"), TEXT("RightHandPinky3"), NAME_None);
	Controller->AddRetargetChain(TEXT("LeftRing"), TEXT("LeftHandRing1"), TEXT("LeftHandRing3"), NAME_None);
	Controller->AddRetargetChain(TEXT("RightRing"), TEXT("RightHandRing1"), TEXT("RightHandRing3"), NAME_None);
	Controller->AddRetargetChain(TEXT("LeftThumb"), TEXT("LeftHandThumb1"), TEXT("LeftHandThumb3"), NAME_None);
	Controller->AddRetargetChain(TEXT("RightThumb"), TEXT("RightHandThumb1"), TEXT("RightHandThumb3"), NAME_None);

#ifndef MAR_IKRETARGETER_IKSOLVERS_DISABLE_

	const int32 SolverIndex = Controller->AddSolver(UIKRigFBIKSolver::StaticClass());
	UIKRigFBIKSolver* Solver = CastChecked<UIKRigFBIKSolver>(Controller->GetSolverAtIndex(SolverIndex));
	...
	if (UIKRig_FBIKBoneSettings* HipsBoneSettings = Cast<UIKRig_FBIKBoneSettings>(Controller->GetBoneSettings(HipsBoneName, SolverIndex)))
	{
	...
	if (UIKRig_FBIKBoneSettings* SpineBoneSettings = Cast<UIKRig_FBIKBoneSettings>(Controller->GetBoneSettings(SpineBoneName, SolverIndex)))
	{
	...
	if (UIKRig_FBIKBoneSettings* Spine1BoneSettings = Cast<UIKRig_FBIKBoneSettings>(Controller->GetBoneSettings(Spine1BoneName, SolverIndex)))
	{
	...
	if (UIKRig_FBIKBoneSettings* Spine2BoneSettings = Cast<UIKRig_FBIKBoneSettings>(Controller->GetBoneSettings(Spine2BoneName, SolverIndex)))
	{
	...
	if (UIKRig_FBIKBoneSettings* LeftShoulderBoneSettings = Cast<UIKRig_FBIKBoneSettings>(Controller->GetBoneSettings(LeftShoulderBoneName, SolverIndex)))
	{
	...
	if (Controller->AddNewGoal(LeftHandGoalName, LeftHandBoneName) == LeftHandGoalName)
	{
		UIKRigEffectorGoal* LeftHandGoal = Controller->GetGoal(LeftHandGoalName);
		check(LeftHandGoal);
		...
		Controller->ConnectGoalToSolver(LeftHandGoalName, SolverIndex);
	...
	if (UIKRig_FBIKBoneSettings* RightShoulderBoneSettings = Cast<UIKRig_FBIKBoneSettings>(Controller->GetBoneSettings(RightShoulderBoneName, SolverIndex)))
	{
	...
	if (Controller->AddNewGoal(RightHandGoalName, RightHandBoneName) == RightHandGoalName)
	{
		UIKRigEffectorGoal* RightHandGoal = Controller->GetGoal(RightHandGoalName);
		check(RightHandGoal);
		...
		Controller->ConnectGoalToSolver(RightHandGoalName, SolverIndex);
	...
	if (UIKRig_FBIKBoneSettings* LeftForeArmBoneSettings = Cast<UIKRig_FBIKBoneSettings>(Controller->GetBoneSettings(LeftForeArmBoneName, SolverIndex)))
	{
	...
	if (UIKRig_FBIKBoneSettings* RightForeArmBoneSettings = Cast<UIKRig_FBIKBoneSettings>(Controller->GetBoneSettings(RightForeArmBoneName, SolverIndex)))
	{
		ConfigureBonePreferredAnglesLocalToBS(
			Skeleton,
	...
	if (UIKRig_FBIKBoneSettings* LeftUpLegBoneSettings = Cast<UIKRig_FBIKBoneSettings>(Controller->GetBoneSettings(LeftUpLegBoneName, SolverIndex)))
	{
		ConfigureBonePreferredAnglesLocalToBS(
			Skeleton,
	...
	if (UIKRig_FBIKBoneSettings* LeftLegBoneSettings = Cast<UIKRig_FBIKBoneSettings>(Controller->GetBoneSettings(LeftLegBoneName, SolverIndex)))
	{
		ConfigureBonePreferredAnglesLocalToBS(
			Skeleton,
			LeftLegBoneSettings,
	...
	if (Controller->AddNewGoal(LeftFootGoalName, LeftToeBaseBoneName) == LeftFootGoalName)
	{
		UIKRigEffectorGoal* LeftFootGoal = Controller->GetGoal(LeftFootGoalName);
		check(LeftFootGoal);
		...
		Controller->ConnectGoalToSolver(LeftFootGoalName, SolverIndex);
	}
	...
	if (UIKRig_FBIKBoneSettings* RightUpLegBoneSettings = Cast<UIKRig_FBIKBoneSettings>(Controller->GetBoneSettings(RightUpLegBoneName, SolverIndex)))
	{
		ConfigureBonePreferredAnglesLocalToBS(
			Skeleton,
	...
	if (UIKRig_FBIKBoneSettings* RightLegBoneSettings = Cast<UIKRig_FBIKBoneSettings>(Controller->GetBoneSettings(RightLegBoneName, SolverIndex)))
	{
		ConfigureBonePreferredAnglesLocalToBS(
			Skeleton,
	...
	if (Controller->AddNewGoal(RightFootGoalName, RightToeBaseBoneName) == RightFootGoalName)
	{
		UIKRigEffectorGoal* RightFootGoal = Controller->GetGoal(RightFootGoalName);
		check(RightFootGoal);
		...
		Controller->ConnectGoalToSolver(RightFootGoalName, SolverIndex);
	}
	...
	{
		Controller->AddRetargetChain(/*ChainName=*/BoneName, /*StartBoneName=*/BoneName, /*EndBoneName=*/BoneName, NAME_None);
	}
...
) const
{
	...
	Controller->AddRetargetChain(TEXT("Root"), TEXT("root"), TEXT("root"), NAME_None);
	Controller->AddRetargetChain(TEXT("Spine"), TEXT("spine_01"), SelectBySkeletonType(SkeletonType, TEXT("spine_03"), TEXT("spine_05")), NAME_None);
	Controller->AddRetargetChain(TEXT("Head"), TEXT("neck_01"), TEXT("head"), NAME_None);
	static const FName LeftClavicleChainName(TEXT("LeftClavicle"));
	Controller->AddRetargetChain(LeftClavicleChainName, TEXT("clavicle_l"), TEXT("clavicle_l"), NAME_None);
	static const FName LeftArmChainName(TEXT("LeftArm"));
	static const FName LeftHandBoneName(TEXT("hand_l"));
	Controller->AddRetargetChain(LeftArmChainName, TEXT("upperarm_l"), LeftHandBoneName, NAME_None);
	static const FName RightClavicleChainName(TEXT("RightClavicle"));
	Controller->AddRetargetChain(RightClavicleChainName, TEXT("clavicle_r"), TEXT("clavicle_r"), NAME_None);
	static const FName RightArmChainName(TEXT("RightArm"));
	static const FName RightHandBoneName(TEXT("hand_r"));
	Controller->AddRetargetChain(RightArmChainName, TEXT("upperarm_r"), RightHandBoneName, NAME_None);
	static const FName LeftLegChainName(TEXT("LeftLeg"));
	static const FName LeftBallBoneName(TEXT("ball_l"));
	Controller->AddRetargetChain(TEXT("LeftLeg"), TEXT("thigh_l"), LeftBallBoneName, NAME_None);
	static const FName RightLegChainName(TEXT("RightLeg"));
	static const FName RightBallBoneName(TEXT("ball_r"));
	Controller->AddRetargetChain(RightLegChainName, TEXT("thigh_r"), RightBallBoneName, NAME_None);
	Controller->AddRetargetChain(TEXT("LeftIndex"), TEXT("index_01_l"), TEXT("index_03_l"), NAME_None);
	Controller->AddRetargetChain(TEXT("RightIndex"), TEXT("index_01_r"), TEXT("index_03_r"), NAME_None);
	Controller->AddRetargetChain(TEXT("LeftMiddle"), TEXT("middle_01_l"), TEXT("middle_03_l"), NAME_None);
	Controller->AddRetargetChain(TEXT("RightMiddle"), TEXT("middle_01_r"), TEXT("middle_03_r"), NAME_None);
	Controller->AddRetargetChain(TEXT("LeftPinky"), TEXT("pinky_01_l"), TEXT("pinky_03_l"), NAME_None);
	Controller->AddRetargetChain(TEXT("RightPinky"), TEXT("pinky_01_r"), TEXT("pinky_03_r"), NAME_None);
	Controller->AddRetargetChain(TEXT("LeftRing"), TEXT("ring_01_l"), TEXT("ring_03_l"), NAME_None);
	Controller->AddRetargetChain(TEXT("RightRing"), TEXT("ring_01_r"), TEXT("ring_03_r"), NAME_None);
	Controller->AddRetargetChain(TEXT("LeftThumb"), TEXT("thumb_01_l"), TEXT("thumb_03_l"), NAME_None);
	Controller->AddRetargetChain(TEXT("RightThumb"), TEXT("thumb_01_r"), TEXT("thumb_03_r"), NAME_None);
	if (bIsUE5Skeleton)
	{
		...
		Controller->AddRetargetChain(TEXT("LeftIndexMetacarpal"), TEXT("index_metacarpal_l"), TEXT("index_metacarpal_l"), NAME_None);
		Controller->AddRetargetChain(TEXT("RightIndexMetacarpal"), TEXT("index_metacarpal_r"), TEXT("index_metacarpal_r"), NAME_None);
		Controller->AddRetargetChain(TEXT("LeftMiddleMetacarpal"), TEXT("middle_metacarpal_l"), TEXT("middle_metacarpal_l"), NAME_None);
		Controller->AddRetargetChain(TEXT("RightMiddleMetacarpal"), TEXT("middle_metacarpal_r"), TEXT("middle_metacarpal_r"), NAME_None);
		Controller->AddRetargetChain(TEXT("LeftPinkyMetacarpal"), TEXT("pinky_metacarpal_l"), TEXT("pinky_metacarpal_l"), NAME_None);
		Controller->AddRetargetChain(TEXT("RightPinkyMetacarpal"), TEXT("pinky_metacarpal_r"), TEXT("pinky_metacarpal_r"), NAME_None);
		Controller->AddRetargetChain(TEXT("LeftRingMetacarpal"), TEXT("ring_metacarpal_l"), TEXT("ring_metacarpal_l"), NAME_None);
		Controller->AddRetargetChain(TEXT("RightRingMetacarpal"), TEXT("ring_metacarpal_r"), TEXT("ring_metacarpal_r"), NAME_None);
	}
	...
	if (UIKRig_FBIKBoneSettings* HipsBoneSettings = Cast<UIKRig_FBIKBoneSettings>(Controller->GetBoneSettings(PelvisBoneName, SolverIndex)))
	{
	...
	if (UIKRig_FBIKBoneSettings* Spine3BoneSettings = Cast<UIKRig_FBIKBoneSettings>(Controller->GetBoneSettings(Spine3BoneName, SolverIndex)))
	{
	...
	if (bIsUE5Skeleton)
	{
		...
		if (UIKRig_FBIKBoneSettings* Spine4BoneSettings = Cast<UIKRig_FBIKBoneSettings>(Controller->GetBoneSettings(Spine4BoneName, SolverIndex)))
		{
		...
		if (UIKRig_FBIKBoneSettings* Spine5BoneSettings = Cast<UIKRig_FBIKBoneSettings>(Controller->GetBoneSettings(Spine5BoneName, SolverIndex)))
		{
	...
	if (UIKRig_FBIKBoneSettings* ClavicleLeftBoneSettings = Cast<UIKRig_FBIKBoneSettings>(Controller->GetBoneSettings(ClavicleLeftBoneName, SolverIndex)))
	{
	...
	if (UIKRig_FBIKBoneSettings* LowerArmLeftBoneSettings = Cast<UIKRig_FBIKBoneSettings>(Controller->GetBoneSettings(LowerArmLeftBoneName, SolverIndex)))
	{
	...
	if (UIKRig_FBIKBoneSettings* ClavicleRightBoneSettings = Cast<UIKRig_FBIKBoneSettings>(Controller->GetBoneSettings(ClavicleRightBoneName, SolverIndex)))
	{
	...
	if (UIKRig_FBIKBoneSettings* LowerArmRightBoneSettings = Cast<UIKRig_FBIKBoneSettings>(Controller->GetBoneSettings(LowerArmRightBoneName, SolverIndex)))
	{
	...
	if (Controller->AddNewGoal(LeftFootGoalName, LeftBallBoneName) == LeftFootGoalName)
	{
		UIKRigEffectorGoal* LeftFootGoal = Controller->GetGoal(LeftFootGoalName);
		check(LeftFootGoal);
	...
	if (Controller->AddNewGoal(RightFootGoalName, RightBallBoneName) == RightFootGoalName)
	{
		UIKRigEffectorGoal* RightFootGoal = Controller->GetGoal(RightFootGoalName);
		check(RightFootGoal);
	...
	if (UIKRig_FBIKBoneSettings* LeftThighBoneSettings = Cast<UIKRig_FBIKBoneSettings>(Controller->GetBoneSettings(LeftThighBoneName, SolverIndex)))
	{
	...
	if (UIKRig_FBIKBoneSettings* RightThighBoneSettings = Cast<UIKRig_FBIKBoneSettings>(Controller->GetBoneSettings(RightThighBoneName, SolverIndex)))
	{
...
) const
{
	...
	*/
	...
	for (URetargetChainSettings* ChainMap : Controller->GetAllChainSettings())
	{
		...
		// Skip if the target chain name is not mapped.
		if (SourceChainName.IsNone())
		{
			// Explicitly unset the SourceChainName because from UE5.2 the automapper is executed by SetIKRig call. 
			Controller->SetSourceChain(NAME_None, ChainMap->TargetChain);
			continue;
		}

		Controller->SetSourceChain(SourceChainName, ChainMap->TargetChain);
		...
		{
			ChainMap->Settings.FK.TranslationMode = ERetargetTranslationMode::GloballyScaled;
		}

#ifndef MAR_IKRETARGETER_IKSOLVERS_DISABLE_
		// Configure the DriveIKGoal setting.
		ChainMap->Settings.IK.EnableIK = TargetBoneChainsDriveIKGoal.Contains(ChainMap->TargetChain);

		if (TargetBoneChainsOneToOneRotationMode.Contains(ChainMap->TargetChain))
		{
			ChainMap->Settings.FK.RotationMode = ERetargetRotationMode::OneToOne;
		}
#endif
	}
